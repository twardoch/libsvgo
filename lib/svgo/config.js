...
import yaml from 'js-yaml' // TODO: drop and only accept resulting export object
...
module.exports = function (config) {
  ...
  if (config.full) {
    ...
    defaults = Object.assign({}, yaml.safeLoad(readFileSync(`${__dirname}/../../.svgo.yml`, 'utf8')))
    defaults.plugins = preparePluginsArray(config, defaults.plugins || [])
...
function loadPlugin (config, name, path) {
  ...
  if (!path) {
    ...
    plugin = require(resolve(config.__DIR, path))
  }
