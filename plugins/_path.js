...
const path2js = function (path) {
  ...
  const paramsLength = { // Number of parameters of every path command
  ...
  path.attr('d').value.split(regPathInstructions).forEach(function (data) {
    ...
    if (regPathInstructions.test(data)) {
      ...
      for (const pair = paramsLength[ instruction ]; data.length;) {
        pathData.push({
 ...
 */
const relative2absolute = function (data) {
 ...
 */
const applyTransforms = function (elem, path, params) {
 ...
 * @return {Object}
 */
const computeCubicBoundingBox = function (xa, ya, xb, yb, xc, yc, xd, yd) {
  ...
  const tsx = computeCubicFirstDerivativeRoots(xa, xb, xc, xd)

  for (let i = 0; i < tsx.length; i++) {
    const t = tsx[ i ]

    if (t >= 0 && t <= 1) {
      const x = computeCubicBaseValue(t, xa, xb, xc, xd)
  ...
  const tsy = computeCubicFirstDerivativeRoots(ya, yb, yc, yd)

  for (let i = 0; i < tsy.length; i++) {
    const t = tsy[ i ]

    if (t >= 0 && t <= 1) {
      // x = computeCubicBaseValue(t, xa, xb, xc, xd);
      const y = computeCubicBaseValue(t, ya, yb, yc, yd)
 ...
 * @return {Object}
 */
const computeQuadraticBoundingBox = function (xa, ya, xb, yb, xc, yc) {
  let minx = Number.POSITIVE_INFINITY
  let miny = Number.POSITIVE_INFINITY
  let maxx = Number.NEGATIVE_INFINITY
  let maxy = Number.NEGATIVE_INFINITY
  ...
  const tx = computeQuadraticFirstDerivativeRoot(xa, xb, xc)

  if (tx >= 0 && tx <= 1) {
    const x = computeQuadraticBaseValue(tx, xa, xb, xc)
  ...
  const ty = computeQuadraticFirstDerivativeRoot(ya, yb, yc)

  if (ty >= 0 && ty <= 1) {
    // x = computeQuadraticBaseValue(t, xa, xb, xc);
    const y = computeQuadraticBaseValue(ty, ya, yb, yc)
...
function a2c (x1, y1, rx, ry, angle, largeArcFlag, sweepFlag, x2, y2, recursive) {
  ...
  let f1
  let f2
  const rotateX = function (x, y, rad) { return x * Math.cos(rad) - y * Math.sin(rad) }
  const rotateY = function (x, y, rad) { return x * Math.sin(rad) + y * Math.cos(rad) }
  if (!recursive) {
