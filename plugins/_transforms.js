...
const matrixToTransform = function (transform, params) {
  ...
  var transforms = []
  var sx = +Math.hypot(data[ 0 ], data[ 1 ]).toFixed(params.transformPrecision)
  ...
  var scaleBefore = rowsSum !== 0 || sx === sy
  ...
  if (!data[ 1 ] && data[ 2 ]) {
    ...
    var rotate = [ mth.acos(angle, floatPrecision) * ((scaleBefore ? 1 : sy) * data[ 1 ] < 0 ? -1 : 1) ]
...
const transformArc = function (arc, transform) {
  ...
  var root = Math.hypot(m[ 0 ] - m[ 3 ], m[ 1 ] + m[ 2 ]) * Math.hypot(m[ 0 ] + m[ 3 ], m[ 1 ] - m[ 2 ])
